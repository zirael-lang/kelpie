...
pub fn find_config<P: AsRef<Path>>(start_path: P, ctx: &mut KelpieContext) -> Result<ProjectId> {
    ...
    loop {
        ...
        if manifest_path.exists() {
            return load_from_manifest(manifest_path, false, ctx);
        }
...
pub fn load_from_manifest<P: AsRef<Path>>(
    ...
    ctx: &mut KelpieContext,
) -> Result<ProjectId> {
    let manifest_path = manifest_path.as_ref().canonicalize()?;

    if let Some(existing_id) = ctx.find_project_by_path(&manifest_path) {
    ...
    let project_id = if let Some(workspace_config) = toml.workspace {
        let member_package_ids = if let Some(members) = workspace_config.members {
            ...
            for member_path in member_paths {
                ...
                let member_project_id = load_from_manifest(&config_path, true, ctx)?;
        ...
        let dependencies = ctx.resolve_dependencies(workspace_config.dependencies.as_ref())?;
        ...
        ctx.add_project(workspace_project, manifest_path)
    } else if let Some(package_config) = toml.package {
        let package_builder = PackageBuilder::from_toml(package_config);
        let package_id = ctx.add_package(package_builder);

        let dependencies = ctx.resolve_dependencies(toml.dependencies.as_ref())?;

        let package_project = Project::new(ProjectKind::Package(package_id), dependencies);

        ctx.add_project(package_project, manifest_path)
    } else {
